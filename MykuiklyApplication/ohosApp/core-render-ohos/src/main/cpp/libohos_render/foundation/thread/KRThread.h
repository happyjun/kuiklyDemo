 ...
 public:
    explicit KRThread(const std::string &name) : m_stop(false) {
        m_workerThread = std::thread([this] {
            ...
            KR_LOG_INFO << "new threrad  kuikly  theradid= "<<m_workerThreadId  ;
            this->Worker();
        });
    ...
    void DispatchAsync(const std::function<void()> task, int delayMilliseconds = 0) {
        if (delayMilliseconds > 0) {
             KR_LOG_INFO << "DispatchAsync post 延迟task 到m_delayThread 的tasks里 ";
        ...
        if (IsCurrentThreadWorkerThread()) {
            ...
            std::unique_lock<std::mutex> lock(m_mutex);
            KR_LOG_INFO << "kuikly thread DispatchAsync  add  task m_tasks old size= "<<m_tasks.size();
            m_tasks.emplace(task);
        } else {
            // 否则，将任务添加到队列
            {
                std::unique_lock<std::mutex> lock(m_mutex);
                 KR_LOG_INFO << "其他线程 DispatchAsync  add  task m_tasks old size= "<<m_tasks.size();
                m_tasks.emplace(task);
            }
            m_condition.notify_one();
        }
    ...
    void DirectRunOnCurThread(const std::function<void()> &task) {
        if (m_isExecutingTask.load()) {
              KR_LOG_INFO << "多线程模式下 DirectRunOnCurThread   m_isExecutingTask true begin  run task ";
            task();
             KR_LOG_INFO << "多线程模式下 DirectRunOnCurThread   m_isExecutingTask true end  run task ";
        } else {
            auto start = std::chrono::steady_clock::now();  // 记录开始时间
            bool didHandleTask = false;
            while (!SyncMainTaskMutex(false, false, false)) {
                 KR_LOG_INFO << "SyncMainTaskMutex";
                // 检查是否超时
                auto now = std::chrono::steady_clock::now();
                auto duration = std::chrono::duration_cast<std::chrono::milliseconds>(now - start);
                if (duration.count() > 100) {
                    KR_LOG_INFO << "超时 直接跳出循环";
                    break;  // 超时，跳出循环
                }
                // 尝试获取互斥锁
                if (TaskMutex(true, false, false)) {
                    m_isExecutingTask.store(true);  // 设置正在执行任务标志
                    KR_LOG_INFO << "DirectRunOnCurThread 任务执行";
                    task();
            ...
            if (!didHandleTask) {
                ...
                KR_LOG_INFO << "DirectRunOnCurThread 内部 else";
            }
        }
    }
        ...
        while (true) {
            std::queue<std::function<void()>> tasks;
            {
                ...
                if (m_stop && m_tasks.empty()) {
                 ...
                 KR_LOG_INFO << "kuikly thread work task  task size = "<<m_tasks.size();

                std::swap(tasks, m_tasks);  // 将m_tasks所有任务一次性移动到tasks
            }
            {
                if (TaskMutex(true, false, false)) {
                     KR_LOG_INFO << "kuikly thread 执行任务  task size = "<<tasks.size();
                    while (!tasks.empty()) {
                        auto &task = tasks.front();
                        KR_LOG_INFO << "kuikly thread 执行任务  开始 ";
                        task();
                          KR_LOG_INFO << "kuikly thread 执行任务  结束 ";
                        tasks.pop();
                    }
                    TaskMutex(false, true, false);
                } else {
                    std::unique_lock<std::mutex> lock(m_mutex);
                    KR_LOG_INFO << "kuikly thread 执行任务  不能 执行 重新pushtask size = "<<tasks.size();
                    while (!tasks.empty()) {
